	...
	public void loanBook(LoanBookReqDto reqDto, LoggedUser loggedUser) {
		...
		if (reqDto.getCount() > availableCopies) {
			...
			if (availableCopies == 0) {
				...
				message = String.format("W systemie nie znajduje się wybrana ilość książek do wypożyczenia. " +
					"Możliwa ilość książek do wypożyczenia: %d.", availableCopies);
			}
		...
		logPersistService.info("Klient: %s wypożyczył książkę: %s.", loggedUser.getUsername(), book.getTitle());
	}
	...
	public void returnBook(ReturnBookReqDto reqDto, LoggedUser loggedUser) {
		...
		if (reqDto.getCount() > count) {
			...
			if (count == 0) {
				...
				message = String.format("Liczba wypożyczonych książek jest mniejsza od zadeklarowanej liczby książek do " +
					"wypożyczenia. Maksymalna liczba wypożyczonych książek: %d.", count);
			}
			throw new RestServerException(message);
		}
		final UserEntity user = userRepository
		...
		logPersistService.info("Klient: %s oddał książkę: %s.", loggedUser.getUsername(), book.getTitle());
	}
